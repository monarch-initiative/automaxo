...
class ProcessJsonToTSV:
    ...
    def extract_passages(self,passages):
        biotype_prefix = {
            "gene": "ncbi",
            "disease": "disease",
            "chemical": "chemical",
    ...
    def extract_relationships(self,relations):
        biotype_prefix = {
            "gene": "ncbi",
            "disease": "disease",
            "chemical": "chemical",
    ...
    def process_jsons_to_tsv(self, _json_files_path, _replaced_tsv_file_path, _no_replaced_tsv_file_path):
        ...
        output_directory = os.path.dirname(_replaced_tsv_file_path)
        if output_directory:
            os.makedirs(output_directory, exist_ok=True)

        # Open the TSV files for writing
        with open(_replaced_tsv_file_path, 'w', encoding='utf-8') as replaced_tsv_file, \
            open(_no_replaced_tsv_file_path, 'w', encoding='utf-8') as no_replaced_tsv_file:

            # Iterate over all files in the given directory
            for filename in os.listdir(_json_files_path):
                if filename.endswith(".json"):
                    json_file_path = os.path.join(_json_files_path, filename)
                    ...
                    # Apply Replacement
                    corrected_passages = self.extract_passages(data["passages"])
                    ...
                    relationship_replaced = self.extract_relationships(data.get('relations', []))
...
python data_preprocessing.py  -i ../dump/json_files/morphine -r ../data/cystic_fibrosis_mesh_replaced.tsv -n ../data/cystic_fibrosis_no_replaced.tsv
